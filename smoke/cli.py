...
def legacyentry():
    ...
    parser_info = subparsers.add_parser(
        'info', help='Show basic SMOKE blockchain info')
    ...
    """
        Command "transfer"
    """
    parser_transfer = subparsers.add_parser('transfer', help='Transfer SMOKE')
    ...
    parser_transfer.add_argument(
        ...
        type=str,
        choices=["SMOKE"],
        help='Asset to transfer (i.e. SMOKE)')
    parser_transfer.add_argument(
    ...
    parser_powerup = subparsers.add_parser(
        'powerup', help='Power up (vest SMOKE as SMOKE POWER)')
    ...
    parser_powerdown = subparsers.add_parser(
        'powerdown',
        help='Power down (start withdrawing SMOKE from SMOKE POWER)')
    ...
    parser_powerdownroute.add_argument(
        ...
        type=str,
        default=configStorage["default_account"],
        help='The account receiving either VESTS/SMOKEPower or SMOKE.')
    parser_powerdownroute.add_argument(
        '--percentage',
        type=float,
        default=100,
    ...
    parser_powerdownroute.add_argument(
        ...
        help=('Set to true if the from account should receive the VESTS as'
              'VESTS, or false if it should receive them as SMOKE.'))
    """
        Command "balance"
    """
    ...
    if args.command == "set":
        ...
        if args.account and isinstance(args.account, list):
            for account in args.account:
                ...
                print("\n%s" % a.name)
                t = PrettyTable(["Account", "SMOKE", "VESTS"])
                t.align = "r"
                t.add_row([
                    'Available',
                    a.balances['available']['SMOKE'],
                    a.balances['available']['VESTS'],
                ])
                t.add_row([
                    'Rewards',
                    a.balances['rewards']['SMOKE'],
                    a.balances['rewards']['VESTS'],
                ])
                t.add_row([
                    'TOTAL',
                    a.balances['total']['SMOKE'],
        ...
        if not args.foreign_account:
            from smokebase.account import PasswordKey
            pwd = get_terminal(
        ...
        if args.account_creation_fee:
            props["account_creation_fee"] = str(
                Amount("%f SMOKE" % args.account_creation_fee))
        ...
        props = {
            "account_creation_fee":
                str(Amount("%f SMOKE" % args.account_creation_fee)),
            "maximum_block_size":
