...
from smokebase.exceptions import (
    PostDoesNotExist,
    VotingInvalidOnArchivedPost,
)
from smokebase.operations import CommentOptions
...
class Post(dict):
    ...
    def refresh(self):
        ...
        for p in sbd_amounts:
            post[p] = Amount(post.get(p, "0.000 SMOKE"))
    ...
    @property
    def reward(self):
        """Return a float value of estimated total SMOKE reward.
        """
        return Amount(self.get("total_payout_value", "0 SMOKE")) + \
               Amount(self.get("pending_payout_value", "0 SMOKE"))
